...
class MidpointSolver {
  ...
  static MidpointResult solve({
    ...
    final pX1 = _parseFraction(x1, 'x₁');
    final pY1 = _parseFraction(y1, 'y₁');
    final pX2 = _parseFraction(x2, 'x₂');
    final pY2 = _parseFraction(y2, 'y₂');
    ...
    final midX = _midOfFractions(a, c);
    final midY = _midOfFractions(b, d);
  ...
  static MidpointResult findEndpointFromMidpoint({
    ...
    final mX = _parseFraction(midpointX, 'Midpoint x');
    final mY = _parseFraction(midpointY, 'Midpoint y');
    final kX = _parseFraction(knownX, 'Known x');
    final kY = _parseFraction(knownY, 'Known y');
    ...
    final fx = _subtractFractions(_multiplyFractionByInt(xm, 2), x1);
    final fy = _subtractFractions(_multiplyFractionByInt(ym, 2), y1);
  ...
  static Fraction _midOfFractions(Fraction a, Fraction b) {
    ...
    // divide by 2
    return _simplify(sumNum, sumDen * 2);
  }

  static Fraction _multiplyFractionByInt(Fraction f, int n) {
    return _simplify(f.numerator * n, f.denominator);
  }

  static Fraction _subtractFractions(Fraction a, Fraction b) {
  ...
  static Fraction _simplify(int n, int d) {
  ...
  ///   "-1/2"  → Fraction(-1, 2)
  ///   "2"     → Fraction(2, 1)
  ///   "1.5"   → Fraction(3, 2)   (decimal → fraction)
  static _FractionParse _parseFraction(String input, String label) {
    final t = input.trim();

    if (t.isEmpty) {
      return _FractionParse.error('$label is required');
    }
    ...
    if (slashMatch != null) {
      ...
      if (d == 0) return _FractionParse.error('$label: denominator cannot be 0');
      return _FractionParse.success(_simplify(n, d));
    }

    // ── Decimal or integer ─────────────────────────────────────
    final dVal = double.tryParse(t);
    if (dVal == null || dVal.isNaN || dVal.isInfinite) {
      return _FractionParse.error('$label must be a number or fraction (e.g. 3/4)');
    }

    return _FractionParse.success(_doubleToFraction(dVal));
  }

  static Fraction _doubleToFraction(double v) {
    if (v == v.toInt()) {
...
class _FractionParse {
  final Fraction? fraction;
  final String? error;

  const _FractionParse({this.fraction, this.error});
  ...
  factory _FractionParse.success(Fraction f) => _FractionParse(fraction: f);
  factory _FractionParse.error(String e) => _FractionParse(error: e);
}