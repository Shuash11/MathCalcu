...
class LinearSolver {
  static SolveResult solve(String input) {
    try {
      ...
      final ops = RegExp(r'[≥≤><]').allMatches(normalized).map((m) => m.group(0)!).toList();
      if (ops.isEmpty) return SolveResult.error('No inequality operator found.');
      ...
      if (sides == null) return SolveResult.error('Could not split expression.');

      final left = InequalityCoreSolver.parseLinear(sides[0]);
      final right = InequalityCoreSolver.parseLinear(sides[1]);
      if (left == null || right == null) {
        return SolveResult.error('Could not parse expression. Check your input.');
      }
      ...
      if (a == 0) {
        ...
        if (satisfies) {
          return const SolveResult(answer: 'All real numbers', points: [], intervalNotation: '(-∞, ∞)');
        } else {
      ...
      return SolveResult(answer: answer, points: [boundary], intervalNotation: interval);
    } catch (e) {
  ...
  static List<StepModel> getSteps(String input) {
    ...
    if (ra != 0) {
      final xPart = a == 1 ? 'x' : (a == -1 ? '-x' : '${InequalityCoreSolver.fmt(a)}x');
      final cPart = lc >= 0
      ...
      steps.add(StepModel(
        ...
        title: 'Group x terms',
        explanation: '',
        latex: '$xPart ${cPart.isNotEmpty ? cPart : ""} $op ${InequalityCoreSolver.fmt(rc)}',
      ));
    }
    ...
    if (a == 0) {
      steps.add(StepModel(
        ...
        explanation: '',
        latex: '${InequalityCoreSolver.fmt(b)} $op 0 → ${InequalityCoreSolver.evalOp(b, op, 0) ? "True: All real numbers" : "False: No solution"}',
      ));
      return steps;
    }
    ...
    if (a == -1) {
      steps.add(StepModel(
        ...
        explanation: '',
        latex: 'x ${InequalityCoreSolver.flipOp(op)} ${InequalityCoreSolver.fmt(boundary)}',
      ));
    } else if (a < 0) {
      steps.add(StepModel(
        stepNumber: n++,
  ...
  static SolveResult _solveDouble(String normalized, List<String> ops, List<String> parts) {
    if (parts.length != 3) return SolveResult.error('Invalid double inequality format.');
    ...
    if (left['x'] != 0 || right['x'] != 0) {
      return SolveResult.error('Variables on the outside are not supported yet.');
    }
    ...
    final lb = (op1 == '≤' || op1 == '≥') ? '[' : '(';
    final rb = (op2 == '≤' || op2 == '≥') ? ']' : ')';
    final interval = '$lb${InequalityCoreSolver.fmt(boundL)}, ${InequalityCoreSolver.fmt(boundR)}$rb';
    
    if (boundL > boundR || (boundL == boundR && (op1 == '<' || op2 == '<'))) {
      return const SolveResult(answer: 'No solution', points: [], intervalNotation: '∅');
    }
    
    final answer = '${InequalityCoreSolver.fmt(boundL)} $op1 x $op2 ${InequalityCoreSolver.fmt(boundR)}';
    return SolveResult(answer: answer, points: [boundL, boundR], intervalNotation: interval);
  }

  static List<StepModel> _stepsDouble(String input, List<String> ops, List<String> parts) {
    final steps = <StepModel>[];
    int n = 1;
    ...
    if (c != 0) {
      final newL = l - c;
      final newR = r - c;
      final xPart = mx == 1 ? 'x' : (mx == -1 ? '-x' : '${InequalityCoreSolver.fmt(mx)}x');
      steps.add(StepModel(
        stepNumber: n++,
        title: c > 0 ? 'Subtract ${InequalityCoreSolver.fmt(c)} from all parts' : 'Add ${InequalityCoreSolver.fmt(-c)} to all parts',
        explanation: '',
        latex: '${InequalityCoreSolver.fmt(newL)} ${ops[0]} $xPart ${ops[1]} ${InequalityCoreSolver.fmt(newR)}',
      ));
    }
    ...
    if (mx != 1) {
      if (mx < 0) {
        ...
        steps.add(StepModel(
          stepNumber: n++,
          title: 'Divide by ${InequalityCoreSolver.fmt(mx)} (flip inequalities)',
          explanation: '',
          latex: '${InequalityCoreSolver.fmt(boundL)} $op1 x $op2 ${InequalityCoreSolver.fmt(boundR)}',
        ));
      } else {
        steps.add(StepModel(
          stepNumber: n++,
