...
class BinOp extends Expr {
  ...
  Expr diff(String variable) {
    switch (op) {
      ...
      case '*':
        ...
        return BinOp('+',
          BinOp('*', left.diff(variable), right),
      ...
      case '^':
        ...
        if (right.isConst) {
          ...
          return BinOp('*', this,
            BinOp('+',
              BinOp('*', right.diff(variable), Func('ln', left)),
  ...
  Expr simplify() {
    ...
    if (sLeft.isConst && sRight.isConst) {
      ...
      switch (op) {
        ...
        case '-': return Num(l - r);
        case '*': return Num(l * r);
        case '/': return r != 0 ? Num(l / r) : this;
        case '^': return Num(pow(l, r).toDouble());
      }
    }
    ...
    switch (op) {
      ...
      case '*':
        ...
        if (sLeft is Num && sRight is Num) return Num(sLeft.value * sRight.value);
        // Combine: n * (n2 * x) -> (n * n2) * x
        if (sLeft is Num && sRight is BinOp && sRight.op == '*' && sRight.left is Num) {
          return BinOp('*', Num(sLeft.value * (sRight.left as Num).value), sRight.right).simplify();
        }
  ...
  @override
  bool hasVar(String variable) => left.hasVar(variable) || right.hasVar(variable);
  ...
  @override
  bool operator ==(Object other) =>
    other is BinOp && op == other.op && left == other.left && right == other.right;
...
class Func extends Expr {
  ...
  Expr diff(String variable) {
    ...
    switch (name) {
      ...
      case 'cot':
        // d/dx[cot(u)] = -csc²(u) * u'
        return BinOp('*', Neg(BinOp('^', Func('csc', arg), const Num(2))), inner);
      case 'sec':
        // d/dx[sec(u)] = sec(u)*tan(u) * u'
        return BinOp('*', BinOp('*', Func('sec', arg), Func('tan', arg)), inner);
      case 'csc':
        // d/dx[csc(u)] = -csc(u)*cot(u) * u'
        return BinOp('*', Neg(BinOp('*', Func('csc', arg), Func('cot', arg))), inner);
      case 'arcsin':
      case 'asin':
        // d/dx[arcsin(u)] = u'/√(1-u²)
        return BinOp('*', 
          BinOp('/', const Num(1), Sqrt(BinOp('-', const Num(1), BinOp('^', arg, const Num(2))))), 
          inner
        );
      case 'arccos':
      case 'acos':
        // d/dx[arccos(u)] = -u'/√(1-u²)
        return BinOp('*', 
          Neg(BinOp('/', const Num(1), Sqrt(BinOp('-', const Num(1), BinOp('^', arg, const Num(2)))))), 
          inner
        );
      case 'arctan':
      case 'atan':
        // d/dx[arctan(u)] = u'/(1+u²)
        return BinOp('*', 
          BinOp('/', const Num(1), BinOp('+', const Num(1), BinOp('^', arg, const Num(2)))), 
          inner
        );
      case 'sinh':
      ...
      case 'sech':
        ...
        return BinOp('*', Neg(BinOp('*', Func('sech', arg), Func('tanh', arg))), inner);
      case 'csch':
        // d/dx[csch(u)] = -csch(u)*coth(u) * u'
        return BinOp('*', Neg(BinOp('*', Func('csch', arg), Func('coth', arg))), inner);
      case 'coth':
        // d/dx[coth(u)] = -csch²(u) * u'
        return BinOp('*', Neg(BinOp('^', Func('csch', arg), const Num(2))), inner);
      case 'ln':
      case 'log':
        // d/dx[ln(u)] = u'/u
  ...
  Expr simplify() {
    ...
    if (sArg.isConst) {
      ...
      try {
        switch (name) {
          case 'sin': return Num(sin(v));
          case 'cos': return Num(cos(v));
          case 'tan': 
          ...
          case 'log': 
            if (v <= 0) return this;
            return Num(log(v));
          case 'exp': return Num(exp(v));
          case 'sqrt': 
            if (v < 0) return this;
            return Num(sqrt(v));
          case 'abs': return Num(v.abs());
          case 'sinh': return Num(_sinh(v));
          case 'cosh': return Num(_cosh(v));
          case 'tanh': return Num(_tanh(v));
          case 'sech': return Num(_sech(v));
          case 'csch': 
    ...
    // sqrt(x^2) = |x|
    if (name == 'sqrt' && sArg is BinOp && sArg.op == '^' && 
  ...
  @override
  bool operator ==(Object other) => other is Func && name == other.name && arg == other.arg;
  ...
  @override
  String format({bool compact = false}) => '$name(${arg.format(compact: compact)})';
}
...
enum TokenType {
  number, variable, operator, lParen, rParen, comma, function, eof
}
...
class Tokenizer {
  ...
  List<Token> tokenize() {
    ...
    while (_pos < input.length) {
      ...
      if (_isDigit(char) || (char == '.' && _pos + 1 < input.length && _isDigit(input[_pos + 1]))) {
        ...
        throw ParseException('Unexpected character "$char" at position $startPos', startPos);
      }
    }
  ...
  Token _readIdentifier(int startPos) {
    ...
    final type = functions.contains(value) ? TokenType.function : TokenType.variable;
...
class Parser {
  ...
  Expr parse() {
    ...
    if (_current < tokens.length && tokens[_current].type != TokenType.eof) {
      throw ParseException('Unexpected token: ${tokens[_current]}', tokens[_current].position);
    }
    return expr;
  }
  ...
  void _expect(TokenType type) {
    if (_peek().type != type) {
      throw ParseException('Expected $type, got ${_peek().type}', _peek().position);
    }
    _advance();
  }
}
...
class DerivativeSolver {
  ...
  static DerivativeSteps _generateSteps(Expr expr, String variable) {
    ...
    steps.add(DerivativeStep(
      ...
      description: 'Find the derivative of the function with respect to $variable',
      expression: expr,
      rule: null
    ));
    ...
    if (expr is BinOp) {
      switch (expr.op) {
        ...
        case '-':
          ...
          break;
        case '*':
          rules.add('Product Rule: d/dx[f(x)·g(x)] = f\'(x)·g(x) + f(x)·g\'(x)');
          _collectRules(expr.left, rules);
          _collectRules(expr.right, rules);
          break;
        case '/':
          rules.add('Quotient Rule: d/dx[f(x)/g(x)] = [f\'(x)·g(x) - f(x)·g\'(x)] / g²(x)');
          _collectRules(expr.left, rules);
          _collectRules(expr.right, rules);
          break;
        case '^':
          if (expr.right.isConst) {
            ...
            rules.add('General Power Rule: d/dx[fᵍ] = fᵍ·(g\'·ln(f) + g·f\'/f)');
          }
          _collectRules(expr.left, rules);
          _collectRules(expr.right, rules);
          break;
      ...
      switch (expr.name) {
        ...
        case 'cosh':
          ...
          break;
        case 'tanh':
          rules.add('Derivative of Hyperbolic Tangent: d/dx[tanh(u)] = sech²(u)');
          break;
        case 'sech':
          rules.add('Derivative of Hyperbolic Secant: d/dx[sech(u)] = -sech(u)·tanh(u)');
          break;
        case 'csch':
          rules.add('Derivative of Hyperbolic Cosecant: d/dx[csch(u)] = -csch(u)·coth(u)');
          break;
        case 'coth':
          rules.add('Derivative of Hyperbolic Cotangent: d/dx[coth(u)] = -csch²(u)');
          break;
        case 'ln':
        case 'log':
