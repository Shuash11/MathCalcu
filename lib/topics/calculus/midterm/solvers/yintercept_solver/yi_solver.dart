...
String eqLatex(YIFraction m, YIFraction b) {
  ...
  return bs.numerator > 0
      ? 'y = $mp + ${fracLatex(bs)}'
      : 'y = $mp - ${fracLatex(bs.abs())}';
}
...
String termStr(int c, String v, {required bool isFirst}) =>
    termLatex(c, v, isFirst: isFirst);
...
int lcm(int a, int b) =>
    a == 0 || b == 0 ? 1 : (a * b).abs() ~/ _gcd2(a.abs(), b.abs());
...
class YInterceptSolver {
  ...
  static YIResult? tryParseSlopeIntercept(
      {required String mText, required String bText}) {
  ...
  static YIResult fromSlopeIntercept(
          {required YIFraction m, required YIFraction b}) =>
      _computeSlopeIntercept(m.simplified(), b.simplified());

  static YIResult fromSlopeInterceptDoubles(
          {required double m, required double b}) =>
      _computeSlopeIntercept(
          YIFraction.fromDouble(m), YIFraction.fromDouble(b));
  ...
  static _Term? _parseTerm(String tok) {
    ...
    if (hasX) {
      varName = 'x';
      coeffStr =
          tok.toLowerCase().replaceAll('x', '').replaceAll('*', '').trim();
    } else if (hasY) {
      varName = 'y';
      coeffStr =
          tok.toLowerCase().replaceAll('y', '').replaceAll('*', '').trim();
    }
    ...
    return _Term(
      variable: varName,
      coeff: YIFraction(
              numerator: sign * coeff.numerator, denominator: coeff.denominator)
          .simplified(),
    );
  }
...
class ParallelPerpendicularSolver {
  ...
  static PPResult _compute(_Line l1, _Line l2) {
    ...
    String lineToLatex(_Line l) =>
        '${l.A}x ${l.B >= 0 ? '+' : '-'} ${l.B.abs()}y ${l.C >= 0 ? '+' : '-'} ${l.C.abs()} = 0';
    ...
    steps.add(PPSolverStep(number: 1, title: 'Identify the equations', blocks: [
      const PPStepBlock(
          ...
          content:
              'Rewrite each equation in general form to identify A, B, C.'),
      const PPStepBlock(
      ...
      PPStepBlock(
          ...
          label: 'Line 1',
          latex:
              '${lineToLatex(l1)} \\Rightarrow A_1=${l1.A}, B_1=${l1.B}, C_1=${l1.C}'),
      PPStepBlock(
          type: PPBlockType.substitution,
          label: 'Line 2',
          latex:
              '${lineToLatex(l2)} \\Rightarrow A_2=${l2.A}, B_2=${l2.B}, C_2=${l2.C}'),
    ]));
    ...
    // Step: Compare slopes
    steps.add(
        PPSolverStep(number: 3, title: 'Extract and compare slopes', blocks: [
      const PPStepBlock(
          type: PPBlockType.note,
          content: 'The slope is the coefficient of x after isolating y.'),
      const PPStepBlock(
          type: PPBlockType.formula,
      ...
      PPStepBlock(
          type: PPBlockType.substitution,
          label: 'Line 1',
          latex: '${si1.latexEquation} \\Rightarrow m_1 = $m1Str'),
      PPStepBlock(
          type: PPBlockType.substitution,
          label: 'Line 2',
          latex: '${si2.latexEquation} \\Rightarrow m_2 = $m2Str'),
      PPStepBlock(
          type: PPBlockType.result, latex: 'm_1 = $m1Str \\qquad m_2 = $m2Str'),
    ]));
  ...
  static List<PPSolverStep> _buildSISteps(
      _Line l1, _SIResult si1, _Line l2, _SIResult si2) {
    PPSolverStep one(_Line l, _SIResult si, int num, String label) {
      ...
      return PPSolverStep(
          ...
          blocks: [
            PPStepBlock(
                type: PPBlockType.note,
                content: 'Isolate y, then divide every term by B=${l.B}.'),
            const PPStepBlock(
                type: PPBlockType.formula,
                latex:
            ...
            PPStepBlock(
                type: PPBlockType.substitution,
                latex:
                    '${l.A}x + ${l.B}y + ${l.C} = 0 \\\\[4pt] ${l.B}y = ${-l.A}x + ${-l.C} \\\\[4pt] y = \\dfrac{${-l.A}}{${l.B}}x + \\dfrac{${-l.C}}{${l.B}}'),
            PPStepBlock(
                type: PPBlockType.working,
                latex:
                    'm = ${fracLatex(mFrac)} \\\\[4pt] b = ${fracLatex(bFrac)}'),
            PPStepBlock(
                type: PPBlockType.result,
                latex: si.latexEquation,
                content: si.equation),
  ...
  static List<PPSolverStep> _buildTestAndConclusion(
    ...
    if (rel == PPRelationship.parallel) {
      ...
      steps.add(testStep(4, 'Apply parallel test', [
        ...
        PPStepBlock(
            type: PPBlockType.substitution,
            latex:
                'm_1 = $m1T = m_2 \\;\\checkmark \\\\[4pt] b_1 = $b1T \\neq b_2 = $b2T \\;\\checkmark'),
        const PPStepBlock(
            type: PPBlockType.result,
            latex: r'\text{Lines are } \textbf{PARALLEL} \;(\parallel)'),
      ]));
      ...
      steps.add(testStep(4, 'Apply perpendicular test', [
        const PPStepBlock(
            type: PPBlockType.note,
            content: 'Perpendicular lines have slopes whose product is -1.'),
        const PPStepBlock(
            type: PPBlockType.formula,
        ...
        PPStepBlock(
            type: PPBlockType.substitution,
            latex:
                '($m1T) \\times ($m2T) = ${fracLatex(product)} = -1 \\;\\checkmark'),
        const PPStepBlock(
            type: PPBlockType.result,
            latex: r'\text{Lines are } \textbf{PERPENDICULAR} \;(\perp)'),
      ]));
      ...
      steps.add(testStep(4, 'Apply coincident test', [
        const PPStepBlock(
            type: PPBlockType.note,
            content:
                'Same slopes AND same y-intercepts means the lines are identical.'),
        const PPStepBlock(
            type: PPBlockType.formula,
        ...
        PPStepBlock(
            type: PPBlockType.substitution,
            latex:
                'm_1 = $m1T = m_2 \\;\\checkmark \\\\[4pt] b_1 = b_2 \\;\\checkmark'),
        const PPStepBlock(
            type: PPBlockType.result,
            latex: r'\text{Lines are the } \textbf{SAME LINE} \;(\equiv)'),
      ]));
    } else {
      ...
      steps.add(testStep(4, 'Apply both tests', [
        const PPStepBlock(
            type: PPBlockType.note,
            content:
                'Check if slopes are equal (parallel) or product = -1 (perpendicular).'),
        const PPStepBlock(
            type: PPBlockType.formula,
            latex:
                r'\text{Parallel: } m_1 = m_2 \qquad \text{Perp: } m_1 \times m_2 = -1'),
        PPStepBlock(
            type: PPBlockType.substitution, latex: 'm_1 = $m1T, m_2 = $m2T'),
        PPStepBlock(
            type: PPBlockType.working,
            latex:
    ...
    steps.add(PPSolverStep(number: 5, title: 'Conclusion', blocks: [
      ...
      PPStepBlock(
          type: PPBlockType.result,
          latex:
              '\\textbf{${_verdictSymbol(rel)}\\;\\text{Lines are ${_verdictString(rel).replaceAll(' ', '\\ ')}}}',
          content:
  ...
  static String _verdictExplanation(PPRelationship r) {
    switch (r) {
      case PPRelationship.parallel:
        return 'Equal slopes, different y-intercepts. Lines never meet.';
      case PPRelationship.perpendicular:
        return 'Slopes are negative reciprocals (product = -1). Lines cross at 90\u00b0.';
      case PPRelationship.sameLine:
        return 'Both slopes and y-intercepts are identical. Lines overlap completely.';
      case PPRelationship.neither:
        return 'Neither condition is satisfied. Lines intersect at an oblique angle.';
    }
  }
}
