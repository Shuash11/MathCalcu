...
class _MidpointScreenState extends State<MidpointScreen> {
  ...
  void _onCalculate() {
    ...
    // Keys are stable field slots (labels repeat, e.g. 'M?' twice).
    final slots = ['fieldA_x', 'fieldA_y', 'fieldB_x', 'fieldB_y'];
    final ctrls = [_aXCtrl, _aYCtrl, _bXCtrl, _bYCtrl];
    final slotLabels = [_fieldAX, _fieldAY, _fieldBX, _fieldBY];
    final slotErrors = <String, String?>{};
    for (var i = 0; i < slots.length; i++) {
      slotErrors[slots[i]] = FieldValidators.number(ctrls[i].text, slotLabels[i]);
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ResponsiveText(_groupALabel,
                              style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: context.watch<ThemeProvider>().accentColor.withValues(alpha: 0.5), letterSpacing: 1.2)),
                          const SizedBox(height: 10.0),
                          _buildInputField(
                            label: _fieldAX,
                            controller: _aXCtrl,
