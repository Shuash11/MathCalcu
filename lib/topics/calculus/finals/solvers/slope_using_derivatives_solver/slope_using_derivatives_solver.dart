...
//
// CAS engine + slope solver (data model lives in slope_model.dart):
...
// Pure data model (Expr AST + SlopeResult) lives in slope_model.dart and
// is re-exported here so existing imports keep working.
import 'slope_model.dart';
export 'slope_model.dart';
...
class Simplifier {
  static Expr simplify(Expr e) {
    ...
    do {
      ...
      c = _once(c);
      i++;
    } while (c.toMathString() != p.toMathString() && i < 20);
    return c;
  }
    ...
    if (l is Num && r is Num) {
      switch (op) {
        ...
        case '*':
          return Num(l.value * r.value);
        case '/':
          if (r.value != 0) return Num(l.value / r.value);
      }
    }
    switch (op) {
      case '+':
        if (l is Num && l.isZero) return r;
        if (r is Num && r.isZero) return l;
        if (r is UnaryNeg) return _simpOp(l, '-', r.operand);
        if (l is UnaryNeg) return _simpOp(r, '-', l.operand);
        break;
      case '-':
        if (l is Num && l.isZero) return UnaryNeg(r);
        if (r is Num && r.isZero) return l;
        if (l.toMathString() == r.toMathString()) return const Num(0);
        if (r is UnaryNeg) return _simpOp(l, '+', r.operand);
        break;
      case '*':
        if (l is Num && l.isZero) return const Num(0);
        if (r is Num && r.isZero) return const Num(0);
        if (l is Num && l.isOne) return r;
  ...
  static (Expr, Expr) extractDerivCoeff(Expr e, String dv) {
    ...
    if (e is UnaryNeg) {
      final (c, r) = extractDerivCoeff(e.operand, dv);
    ...
    if (e is BinOp) {
      ...
      if (e.op == '*') {
        final lH = ExprUtils.containsDerivSym(e.left),
            rH = ExprUtils.containsDerivSym(e.right);
        if (!lH && rH) {
          final (rc, rr) = extractDerivCoeff(e.right, dv);
          return (
      ...
      if (e.op == '/') {
        final nH = ExprUtils.containsDerivSym(e.left),
            dH = ExprUtils.containsDerivSym(e.right);
        if (!dH && nH) {
...
class Differentiator {
  ...
  static Expr _diffPow(Pow e, String v, Set<String> dep) {
    final bH = ExprUtils.containsVar(e.base, v) || _hasDep(e.base, dep),
        eH = ExprUtils.containsVar(e.exponent, v) || _hasDep(e.exponent, dep);
  ...
  static bool _hasDep(Expr e, Set<String> dep) {
    for (final d in dep) {
      if (ExprUtils.containsVar(e, d)) return true;
    }
    return false;
  }
  ...
  static Expr _diffFunc(Func e, String v, Set<String> dep) {
    ...
    switch (e.name) {
      case 'sin':
        ...
        break;
      case 'cos':
        od = UnaryNeg(Func('sin', u));
        break;
      case 'tan':
        od = BinOp(const Num(1), '/', Pow(Func('cos', u), const Num(2)));
        break;
      case 'cot':
        od = UnaryNeg(
      ...
      case 'abs':
        ...
        break;
      default:
        throw Exception('Cannot diff: ${e.name}');
    }
...
class SlopeSolver {
  ...
  static SlopeResult _solveExplicit(
    ...
    if (pv.containsKey(iv)) {
      final xv = pv[iv]!;
      try {
        sv = ExprUtils.evaluate(sd, pv);
        ts = sv;
        final yv = ExprUtils.evaluate(r, pv);
        ty = yv - ts * xv;
  ...
  static SlopeResult _solveImplicit(
    ...
    if (pv.containsKey('x') && pv.containsKey('y')) {
      try {
        sv = ExprUtils.evaluate(iS, pv);
        ts = sv;
        final xv = pv['x']!, yv = pv['y']!;
        final yi = yv - ts * xv;
        te = _lineEq(ts, yi, xv, yv);
        if (ts != 0 && ts.isFinite) {
          ns = -1.0 / ts;
          final ny = yv - ns * xv;
          ne = _lineEq(ns, ny, xv, yv);
  ...
  static SlopeResult _solveParametric(String input, Map<String, double> pv) {
    ...
    for (final part in parts) {
      final toks = Tokenizer(part.trim()).tokenize();
      final (l, r) = Parser(toks).parse();
    ...
    if (pv.containsKey(pv2)) {
      try {
        final tv = pv[pv2]!;
        final xv = ExprUtils.evaluate(xE, pv), yv = ExprUtils.evaluate(yE, pv);
        final dXv = ExprUtils.evaluate(dx, pv),
            dYv = ExprUtils.evaluate(dy, pv);
        if (dXv == 0) {
          te = 'x = ${_fmt(xv)} (vertical at $pv2=${_fmt(tv)})';
        } else {
          sv = dYv / dXv;
          ts = sv;
          final yi = yv - ts * xv;
          te = '${_lineEq(ts, yi, xv, yv)}  [at $pv2=${_fmt(tv)}]';
          if (ts != 0 && ts.isFinite) {
            ns = -1.0 / ts;
            final ny = yv - ns * xv;
