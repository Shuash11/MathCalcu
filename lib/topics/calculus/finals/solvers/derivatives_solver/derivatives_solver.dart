...
class Num extends Expr {
  ...
  @override
  Expr diff(String v) => const Num(0);
  @override
  Expr simplify() => this;
  @override
  bool hasVar(String v) => false;
  @override
  ...
  @override
  String toString() => value == value.truncateToDouble()
...
class Var extends Expr {
  ...
  const Var(this.name);
  @override
  Expr diff(String v) => Num(name == v ? 1.0 : 0.0);
  @override
  Expr simplify() => this;
  @override
  bool hasVar(String v) => name == v;
  @override
...
class BinOp extends Expr {
    ...
    if (l.isConst && r.isConst) {
      ...
      switch (op) {
        ...
        case '*':
          return Num(lv * rv);
        case '/':
          return rv != 0 ? Num(lv / rv) : this;
        case '^':
          return Num(math.pow(lv, rv).toDouble());
      }
    }
    switch (op) {
      case '+':
  ...
  @override
  bool hasVar(String v) => left.hasVar(v) || right.hasVar(v);
  @override
...
class Neg extends Expr {
  ...
  const Neg(this.expr);
  @override
  Expr diff(String v) => Neg(expr.diff(v));
  @override
  Expr simplify() {
  ...
  @override
  bool hasVar(String v) => expr.hasVar(v);
  @override
...
class Func extends Expr {
  ...
  Expr diff(String v) {
    ...
    switch (name) {
      case 'sin':
        ...
        break;
      case 'cos':
        outer = Neg(Func('sin', arg));
        break;
      case 'tan':
        outer = BinOp('^', Func('sec', arg), const Num(2));
        break;
      case 'sec':
        outer = BinOp('*', Func('sec', arg), Func('tan', arg));
        break;
      case 'csc':
        outer = Neg(BinOp('*', Func('csc', arg), Func('cot', arg)));
        break;
      case 'cot':
        outer = Neg(BinOp('^', Func('csc', arg), const Num(2)));
        break;
      case 'exp':
        outer = this;
        break;
      case 'ln':
        outer = BinOp('/', const Num(1), arg);
        break;
      case 'log':
        outer = BinOp(
  ...
  Expr simplify() {
    ...
    if (s.isConst) {
      ...
      try {
        switch (name) {
          case 'exp':
            return Num(math.exp(v));
          case 'sqrt':
            return v >= 0 ? Num(math.sqrt(v)) : this;
          case 'abs':
            return Num(v.abs());
          case 'sin':
            return Num(math.sin(v));
          case 'cos':
            return Num(math.cos(v));
          case 'tan':
            return Num(math.tan(v));
          case 'ln':
            return v > 0 ? Num(math.log(v)) : this;
        }
      } catch (_) {
        return this;
      }
  ...
  @override
  bool hasVar(String v) => arg.hasVar(v);
  @override
...
class Sqrt extends Expr {
  ...
  @override
  Expr diff(String v) =>
      BinOp('/', arg.diff(v), BinOp('*', const Num(2), Sqrt(arg)));
  @override
  Expr simplify() {
    final s = arg.simplify();
    if (s.isConst) {
...
class Abs extends Expr {
  ...
  const Abs(this.arg);
  @override
  Expr diff(String v) => BinOp('*', arg.diff(v), BinOp('/', arg, Abs(arg)));
  @override
  Expr simplify() {
    final s = arg.simplify();
    if (s.isConst) return Num(s.constValue!.abs());
    return Abs(s);
  }
...
class ParseException implements Exception {
  ...
  @override
  String toString() => 'ParseException: $message at $pos';
}
  ...
  List<Token> tokenize() {
    ...
    while (_pos < input.length) {
      ...
      if (_isDigit(char) ||
        ...
        throw ParseException('Unexpected "$char" at $start', start);
      }
    }
  ...
  Expr _primary() {
    ...
    switch (t.type) {
      ...
      case TokenType.lParen:
        ...
        return e;
      case TokenType.eof:
        throw ParseException('Unexpected end', t.pos);
      default:
        throw ParseException('Unexpected: $t', t.pos);
    }
  }
  ...
  static String _determineRule(Expr e) {
    ...
    if (e is Func) {
      ...
      switch (e.name) {
        ...
        case 'sec':
          return 'Secant Derivative';
        case 'csc':
          return 'Cosecant Derivative';
        case 'cot':
          return 'Cotangent Derivative';
        case 'exp':
          return 'Exponential Derivative';
        case 'ln':
          return 'Natural Log Derivative';
        case 'sqrt':
