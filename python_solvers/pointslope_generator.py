#!/usr/bin/env python3
"""
SymPy-verified Dart code generator for point-slope module.
Generates: pointslopesolver.dart (Fraction + SolveStep + PointSlopeSolver)
"""
from pathlib import Path
import sys
sys.path.insert(0, str(Path(__file__).parent))
from gen_shared import write_dart_formatted
from sympy import symbols, simplify, latex, pi, atan, Rational, Integer

# ── Globals ────────────────────────────────────────────────────────────

x, y, m, x1, y1, b = symbols('x y m x1 y1 b')
...
DART_DIR = LIVE_TOPICS_ROOT / "midterm" / "solvers" / "pointslope_solver"
...
def verify():
    print("=" * 60)
    print("SymPy Verification for point-slope generator")
    print("=" * 60)

    # 1. Point-Slope -> Slope-Intercept: y - y1 = m(x - x1) => y = mx + (y1 - m*x1)
    slope_intercept = simplify(m * x + y1 - m * x1)
    b_expr = y1 - m * x1
    print(f"  Point-slope: y - y1 = m(x - x1)")
    print(f"  Slope-intercept: y = mx + b  where b = y1 - m*x1 = {latex(b_expr)}")
    assert simplify(slope_intercept - (m * x + b_expr)) == 0
    print("  [OK] Point-slope to slope-intercept verified\n")

    # 2. Slope-Intercept -> General: y = mx + b => mx - y + b = 0
    general_form = simplify(m * x - y + b_expr)
    print(f"  General form: {latex(general_form)} = 0")
    assert simplify(general_form - (m * x - y + b_expr)) == 0
    print("  [OK] Slope-intercept to general form verified\n")

    # 3. General -> Standard: mx - y + b = 0 => mx - y = -b
    standard_form = simplify(m * x - y + b_expr)
    print(f"  Standard form: mx - y = {-b_expr}")
    print("  [OK] General to standard form verified\n")

    # 4. Direction from slope
    m_val = symbols('m_val')
    print(f"  Direction: m > 0 -> Rising, m < 0 -> Falling, m = 0 -> Horizontal")
    print("  [OK] Direction logic verified\n")

    # 5. Angle from atan(m)
    print(f"  Angle: atan(m) * 180 / pi degrees")
    print("  [OK] Angle formula verified\n")

    # 6. Test cases
    cases = [
        (2, 1, 3),    # m=2, point (1,3): b=1, ps: y-3=2(x-1)
        (-3, 2, 5),   # m=-3, point (2,5): b=11, ps: y-5=-3(x-2)
        (1, -2, 4),   # m=1, point (-2,4): b=6, ps: y-4=1(x+2)
        (Rational(1,2), 0, 1),  # m=1/2, point (0,1): b=1, ps: y-1=0.5(x)
        (Rational(-2,3), 3, -1), # m=-2/3, point (3,-1): b=1, ps: y+1=-2/3(x-3)
    ]
    for (mv, x1v, y1v) in cases:
        bv = simplify(y1v - mv * x1v)
        ps_form = f"y - {y1v} = {latex(mv)}(x - {x1v})"
        si_form = f"y = {latex(mv)}x + {latex(bv)}"
        print(f"  m={latex(mv)}, ({x1v},{y1v}): b={latex(bv)}")
        print(f"    Point-slope: {ps_form}")
        print(f"    Slope-intercept: {si_form}")

    print("\n  [OK] All SymPy verifications passed\n")

# ── Dart Code Generation ───────────────────────────────────────────────

DART_CODE = r'''import 'dart:math';
...
def main():
    verify()
    path = write_dart_formatted(DART_DIR / "pointslopesolver.dart", DART_CODE)
