#!/usr/bin/env python3
"""
SymPy-verified Dart code generator for Limits at Infinity module.
Generates: limits_infinity_solver.dart
...
def verify():
    print("=" * 60)
    print("SymPy Verification for limits_infinity generator")
    print("=" * 60)

    cases = [
        # (expr_str, approach, expected_str, desc)
        ("(3*x^2 + 2)/(x^2 + 1)", "oo", "3", "Rational: equal degrees -> ratio of leading coeffs"),
        ("(x^2 + 1)/(x^3 + x)", "oo", "0", "Rational: denominator degree > numerator -> 0"),
        ("(x^3 + 2*x)/(x^2 + 1)", "oo", "oo", "Rational: numerator degree > denominator -> oo"),
        ("(2*x^2 - x)/(x^2 + 5)", "oo", "2", "Rational: equal degrees -> 2"),
        ("(x^2 + 1)/(x^3 + x)", "-oo", "0", "Rational: denominator degree > numerator -> 0 at -oo"),
        ("x^2", "oo", "oo", "Polynomial: degree 2 -> oo"),
        ("-x^3", "oo", "-oo", "Polynomial: negative leading coeff, odd degree -> -oo"),
        ("x^3", "-oo", "-oo", "Polynomial: odd degree -> -oo at -oo"),
        ("x^2", "-oo", "oo", "Polynomial: even degree -> oo at -oo"),
        ("5", "oo", "5", "Constant: -> itself"),
        ("(x + 1)/(x - 1)", "oo", "1", "Rational: equal degrees -> 1"),
        ("(2*x^3 - x)/(x^3 + 1)", "oo", "2", "Rational: equal degrees -> 2"),
    ]

    for expr_str, approach_str, expected_str, desc in cases:
        try:
            sym_expr = parse_expr(expr_str.replace("^", "**"))
            approach = oo if approach_str == "oo" else (-oo if approach_str == "-oo" else float(approach_str))
            result = limit(sym_expr, x, approach)
            expected = parse_expr(expected_str.replace("^", "**"))
            ok = simplify(result - expected) == 0
            status = "OK" if ok else f"MISMATCH (got {latex(result)})"
        except Exception as e:
            status = f"ERROR: {e}"
        print(f"  limit({expr_str}, x->{approach_str}) = {expected_str}  [{status}]")
